...
\title{Early leafout leads to longer cooler season growing seasons for woody species} % Phenological differences among species explain why early leafout extends the calendar but not thermal growing season
...
Here, we examine how start-of-season events (i.e., leafout) may affect end-of-season events (i.e., budset) to determine the length of the growing season. Using rarely available plant-scale data---phenological observations over three years from a multi-species common garden study---we test how correlated leafout and budset are in different species from four populations across a 4.5$\degree$ latitudinal cline and examine which phenological event more strongly influences variation in growing season length. From this, we can understand how start- and end-of-season events together impact the calendar growing season, and how this relates to the thermal growing season, which connects to potential productivity. This study offers insights into physiology that will allow us to scale from ecosystem level observations to individual mechanisms too, and improve forecasting. % emwJuly22: last two sentences here need some flow help. DMB: Agreed, want to take a stab at it?

\section{Results \& Discussion} 
\subsection{Variation in leafout and budset}
% dlAug21: Would it help the reader for the details discussed in the last three sentences to be earlier and intersperced in the paragraph to follow directly after when you report the model estimates for the species, year and population?
Our multi-species common garden captured high variation in leafout and budset, %and leaf coloration, 
allowing us to examine how these phases correlate, but also providing important insights into how both vary across species, populations and years. Consistent with the high number of studies that have found species and year-to-year environmental variation drive leafout variation \citep{delpierre2024, donnelly2017, polgar2011}, we found high variance in leafout timing among species and years ($\sigma_{species}$:\Sexpr{sig.sp} $UI_{95}$[\Sexpr{sig.sp.l},\Sexpr{sig.sp.h}], $\sigma_{year}$:\Sexpr{sig.yr} $UI_{95}$[\Sexpr{sig.yr.l},\Sexpr{sig.yr.h}], Figure \ref{fig:vapar}a,b). Population level variation was low ($\sigma_{population}$:\Sexpr{sig.pop}, $UI_{95}$[\Sexpr{sig.pop.l},\Sexpr{sig.pop.h}], Figure \ref{fig:vapar}b). \emph{Sambucus racemosa} was typically the first species to leafout in the spring, leafing out approximately two weeks before \emph{Sorbus americana}, the last species to leaf out (Figure \ref{fig:vapar}a). %There were no differences in leafout timing among the four populations included in our study (Figure \ref{fig:vapar}b). 
Leafout was the earliest in 2019 and the latest in 2020 (Figure \ref{fig:vapar}c). 


Spring phenological phases are reported to be more plastic than autumn ones \citep{mckown2014, aitken2016, vico2021}, but we found that, relative to leafout, variance in budset %and leaf coloration 
timing was higher for species, years and populations ($\sigma_{species}$:\Sexpr{bs.sig.sp} $UI_{95}$[\Sexpr{bs.sig.sp.l},\Sexpr{bs.sig.sp.h}], $\sigma_{year}$:\Sexpr{bs.sig.yr} $UI_{95}$[\Sexpr{bs.sig.yr.l},\Sexpr{bs.sig.yr.h}], $\sigma_{population}$:\Sexpr{bs.sig.pop}, $UI_{95}$[\Sexpr{bs.sig.pop.l},\Sexpr{bs.sig.pop.h}], Figure \ref{fig:vapar}), but followed similar relative contributions (highest variance in year, lowest in population). Budset was earliest for \emph{Amelanchier canadensis} and latest for \emph{Alnus incana} and \emph{Betula paperifera} with more than three weeks between them (Figure \ref{fig:vapar}a). %Leaf coloration was earliest for \emph{Myrica gale} and latest for \emph{Alnus incana}. 
...
These results are somewhat surprising as autumn phenological phases like budset % and leaf coloration 
...
%Budset and leaf coloration phenology were generally correlated (put coefficient in here), but we observed notable species-level variation. For example species like \emph{Sambucus racemosa}, \emph{Spirea tomentosa} were about the lastest to set but had earlier leaf coloration relative the the rest of the community. Other species like \emph{Betula populifolia} and \emph{Alnus incana} had both later budset and leaf coloration relative to the community. These differences may reflect %dlAug21: typo
%dlAug21: what is a primary growing season vs full? should this concept be defined in the intro?
High variation across species in their leafout and budset lead to species driving the most variation in growing season length  ($\sigma_{species}$:\Sexpr{gs.sig.sp}, $UI_{95}$[\Sexpr{gs.sig.sp.l},\Sexpr{gs.sig.sp.h}]Figure \ref{fig:vapar}a), with less variation among years ($\sigma_{year}$:\Sexpr{gs.sig.yr}, $UI_{95}$[\Sexpr{gs.sig.yr.l},\Sexpr{gs.sig.yr.h}], Figure \ref{fig:vapar}c) and little variation explained by population ($\sigma_{population}$:\Sexpr{gs.sig.pop}, $UI_{95}$[\Sexpr{gs.sig.pop.l},\Sexpr{gs.sig.pop.h}], Figure \ref{fig:vapar}b). Due to it's early leafout and late budset \emph{S.racemosa} had the longest calendar growing season of the species in our study. % while \emph{Spirea alba} had the longest full growing season due to early leafout and late leaf coloration. 
\emph{A. canadensis} and \emph{S. americana} had the shortest growing seasons, though for \emph{A. canadensis} this was due to early budset and for \emph{S. americana} late leafout (Figure \ref{fig:vapar}a). %\emph{Myrica gale} had the shortest full growing season due to early leaf coloration despite it's average leafout timing. 
...
% emwJuly22: Do you want to make the below nums Sexpr? DB probably
For individuals, a later start of spring (i.e., leafout) in calendar time was associated with shorter calendar growing seasons (mean estimated effect: $-0.44, UI_{95}[-0.73, -0.16]$). This trade-off between leafout and calendar growing season was apparent in all species in our common garden, but to varying degrees (Fig: \ref{fig:thermcal}a,b). %When we controlled for species and population level differences, however, we found earlier leafout led to earlier budset (Pearson's correlation coefficient of \Sexpr{corrr} $CI_{95}$[\Sexpr{corrr.low}, \Sexpr{corrr.high}]), resulting in a relatively stable calendar growing season \ref{fig:vapar}c). % emwJuly22: Can we spell out what we mean here? Add a sentence at the end? DBSep17 okay, I know the problem, the varience parititioning model shows stable growing seasons across years (Fig1), with this model figure (Fig2) shows that eairier leafout make for a longer season. I was trying to highlight that difference, but I think it maybe it works by just adding individuals, but let me know.
...
% emwJuly22: I would move this to the caption of the figure and just reference it in a sentence above. I would move the first paragraph of the next section UP into this section. DB Sept17 I am not exactuly sure what you mean by "this" here but feel free to do it.
...
%dlAug21: could we elaborate on what is meant by "well-established critical needs"?
