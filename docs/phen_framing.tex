...
% Dan: Get a word count of abstract to refs (and also get a ref count) ... how far off are we from https://github.com/vvandermeersch/synchrony/issues/4 
  %DB says: abstract needs to be 70 word, currently at 67 but I feel like we need one more sentece explaining the significance?
        ...
        %way over with refs currently at 46, and the guideline is 20
...
Beyond species-level variation, our results also found high variation across years in both leafout and budset. Differences across years in leafout were high ($\sigma_{year}$:10.65 days $UI_{90}$[4.88,20.86], Figure \ref{fig:vapar}) and expected, as leafout is highly plastic, varying with local climatic conditions---one of the reasons it is the most consistent biological indicator of anthropogenic climate change \citep{ipcc2022}. In line with this, we also found almost no variation across populations in leafout ($\sigma_{population}$:0.57, $UI_{95}$[0.04,1.77], Figure \ref{fig:vapar}b). Yet the finding of strong variation across years in budset was surprising (\sigma_{year}$:15.02 days $UI_{90}$[6.44,30.84], Figure \ref{fig:vapar}), given budset is generally thought to be strongly locally adapted and controlled by photoperiod, which does not vary year to year \citep{soolanayakanahally2013timing,aitken2016,vico2021}. We did find evidence of local adaptation ($\sigma_{population}$:2.34 days, $UI_{90}$[0.4,6.13], Figure \ref{fig:vapar}). Populations from the Second College Grant (the site with the fewest frost free days; Figure \ref{fig:ffd}) set buds approximately two days before those from the White Mountains (the site with the next fewest frost free days). These differences, however, were weak (Figure \ref{fig:vapar}b) and 6X and \Sexp{spVpop}X smaller than the variation due to year and species (respectively). 
